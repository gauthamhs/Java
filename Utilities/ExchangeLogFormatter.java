...
/**
 * Code copied from DefaultExchangeFormatter and modified to make more splunk friendly.
 *
 */
public class ExchangeLogFormatter implements ExchangeFormatter {
    ...
    private String styleEquals(String label) {
        if (style == OutputStyle.Default) {
            return String.format(", %s=", label);
        }
        if (style == OutputStyle.Tab) {
    ...
    private String skipBodyLineSeparator(String body) {
        body = StringHelper.replaceAll(body, LS, "");
        if (!NL.equals(LS)) {
            body = StringHelper.replaceAll(body, NL, "");
        }
        return body;
    }
    ...
    public String format(Exchange exchange) {
        ...
        if (showAll || showExchangeId) {
            ...
            sb.append(styleEquals("ExchangeId")).append(exchange.getExchangeId());
        }
        if (showAll || showExchangePattern) {
            if (multiline) {
                sb.append(SEPARATOR);
            }
            sb.append(styleEquals("ExchangePattern")).append(exchange.getPattern());
        }
        ...
        if (showAll || showBodyType) {
            ...
            sb.append(styleEquals("BodyType")).append(getBodyTypeAsString(in));
        }
        if (showAll || showBody) {
            if (multiline) {
                sb.append(SEPARATOR);
            ...
            if (skipBodyLineSeparator) {
                body = skipBodyLineSeparator(body);
            }
        ...
        if (showAll || showException || showCaughtException) {
            ...
            if (exception != null) {
                ...
                if (caught) {
                    sb.append(styleEquals("CaughtExceptionType")).append(exception.getClass().getCanonicalName());
                    sb.append(styleEquals("CaughtExceptionMessage")).append(exception.getMessage());
                } else {
                    sb.append(styleEquals("ExceptionType")).append(exception.getClass().getCanonicalName());
                    sb.append(styleEquals("ExceptionMessage")).append(exception.getMessage());
                }
        ...
        if (showAll || showOut) {
            if (exchange.hasOut()) {
                ...
                if (showAll || showBodyType) {
                    ...
                    sb.append(styleEquals("OutBodyType")).append(getBodyTypeAsString(out));
                }
                if (showAll || showBody) {
                    if (multiline) {
                        sb.append(SEPARATOR);
